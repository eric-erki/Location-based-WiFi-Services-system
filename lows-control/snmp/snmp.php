...
class snmp
{
  ...
  */
  function get($host, $target, $security=NULL)
  {
    ...
    return array_shift($this->exec($host, 'get', $varbind, $security));
  }
  ...
  */
  ...
  {
    return array_shift($this->exec($host, 'getbulk', $this->build_varbind($oids), $security));
  }
 ...
 /**
  * get next oid from a single host
  *
  * @param string $host hostnames or ip addresses
  * @param string $oid (must be numeric)
  * @param array $security parameters
  * @return array in the format $oid=>$value
  */
  function get_next($host, $oid, $security=NULL)
  {
    return array_shift($this->exec($host, 'getnext', $this->build_varbind($oid), $security));
  }

 /**
  * get next oid from multiple hosts
  *
  * @param array $hosts hostnames or ip addresses
  * @param string $oid (must be numeric)
  * @param array $security parameters
  * @return array in the format $ip=>array($oid=>$value)
  */
  function multi_get_next($hosts, $oid, $security=NULL)
  {
    return $this->exec($hosts, 'getnext', $this->build_varbind($oid), $security);
  }
  ...
  */
  ...
  {
    return array_shift($this->exec($host, 'getnext', $this->build_varbind($oid), $security, $oid));
  }
    ...
    while(count($queue))
    {
      do
      {
        ...
        if($buffer != '' && isset($ret[$rhost]))
        {
          ...
          if($msg->version() == SNMP_VERSION_3 && $security['v3_context_engine_id'] == '')
          {
          ...
          else
          {
            ...
            foreach($pdu->varBindList() as $val)
            {
              ...
              if(($stop == '' || strpos($oid, $stop) !== false) && !isset($ret[$rhost][$oid]))
              {
                if($type == 'getnext' && $stop != '')
